...
public class Week03ArraysAndMethodsLab {
	...
	public static void main(String[] args) {
		...
		for(int element : arr)System.out.println("element :" + element);
		...
		int summOfLetters = 0;
		for(String name : names) summOfLetters += name.length();
...
;		}
	...
	private static boolean evenSumIsGreater(String[] arr) {
		...
		int oddSum = 0;
		for(String s: arr) {
			if(s.length() % 2 == 0) evenSum += s.length();
			else oddSum += s.length();
		}
